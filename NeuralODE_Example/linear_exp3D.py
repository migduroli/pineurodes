...
from plotter3D import plot_trajectories
...
if __name__ == '__main__':
    ...
    LinearFunctionExample = LinearODEF(Tensor([[-1., 0, 0], [0, -1., 0], [0, 0, -1.]]))
    RandomLinearODEF = LinearODEF(Tensor([[-1, 0, -1], [-0.5, -1, 0], [0, 0.5, -0.5]]))
    ...
    # noinspection PyArgumentList
    z0 = Variable(torch.Tensor([[1, 1, 1]]))
    ...
    for i in range(n_step):
        ...
        if i % plot_freq == 0:
            ...
            plot_trajectories(obs=[obs], times=[times], trajs=[z_p], save=f"imgs3D/{i}.png")
    ...
    plt.savefig('imgs3D/loss_graph.png')
    plt.close()
