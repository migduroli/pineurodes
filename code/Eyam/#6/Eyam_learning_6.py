...
parser.add_argument('--data_size', type=int, default=123)
...
r_new = torch.Tensor(261-s_itp(t)-i_itp(t))
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        loss = torch.mean(torch.abs(pred_y - batch_y)) + 1*(torch.abs(torch.mean(torch.sum(pred_y, 2))-261))
        loss.backward()
        optimizer.step()
        ...
        if itr % args.test_freq == 0: # or itr == 1:
            with torch.no_grad():
                ...
                loss_test = torch.mean(torch.abs(pred_test - test_y)) + 1*(torch.abs(torch.mean(torch.sum(pred_test, 2))-261))
