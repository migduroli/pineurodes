...
parser = argparse.ArgumentParser('LV_learning')
...
equationdata = np.loadtxt('LV_data.txt')
...
def visualize(time, true_y, pred_y, itr):
    if args.viz:
        ...
        ax1.cla()
        ax1.plot(time.numpy(), true_y.numpy()[:, 0, 0], 'bo', label='Original Prey', markersize=2)
        ax1.plot(time.numpy(), pred_y.numpy()[:, 0, 0], 'b--', label='Prediction Prey')
        ax1.plot(time.numpy(), true_y.numpy()[:, 0, 1], 'ro', label='Original Predator', markersize=2)
        ax1.plot(time.numpy(), pred_y.numpy()[:, 0, 1], 'r--', label='Prediction Predator')
        ax1.set_xlabel('Time', fontsize=14)
        ax1.set_ylabel('Numbers', fontsize=14)
        ax1.set_xlim(-0.3, 10.3)
        ax1.set_ylim(-0.3, 7.3)
        ax1.text(-1.8, 6.8, '(a)', fontsize=20)
        ax1.tick_params(axis='x', labelsize=13)
        ax1.tick_params(axis='y', labelsize=13)

        ax2.cla()
        ax2.plot(true_y.numpy()[:, 0, 0], true_y.numpy()[:, 0, 1], 'ko', label='Original', markersize=2)
        ax2.plot(pred_y.numpy()[:, 0, 0], pred_y.numpy()[:, 0, 1], 'k--', label='Prediction')
        ax2.set_xlabel("Prey", fontsize=14)
        ax2.set_ylabel("Predator", fontsize=14)
        ax2.tick_params(axis='x', labelsize=13)
        ax2.tick_params(axis='y', labelsize=13)
        ax2.set_xlim(-0.3, 7.9)
        ax2.set_ylim(-0.3, 5.3)
        ax2.text(-1.8, 4.9, '(b)', fontsize=20)
